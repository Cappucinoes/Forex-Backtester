...
def fire_up(acc_id, access_t):
    ...
    params = {"count":300, # sets parameters for query
                "granularity": "M1"}
    writer = pd.ExcelWriter("/Users/Cappucinoes/PycharmProjects/Bakalarka_PA/range_market_multi/df_hotovy.xlsx")
                    ...
                    if float(df_hotovy.c[valid_range_index + i]) > df_hotovy.roof_price[valid_range_index]:  # BUY SIGNAL
                        for potencial_void in range(5):
                            if df_hotovy.candle_size_pip[valid_range_index + i + potencial_void] > (
                                ...
                                signal = "BUY {} SL {} TP {}".format(df_hotovy.roof_price[valid_range_index], df_hotovy.floor_price[valid_range_index] - 0.00020,
                                                                     df_hotovy.roof_price[valid_range_index] + (abs(df_hotovy.roof_price[valid_range_index] - (df_hotovy.floor_price[valid_range_index] - 0.00020))) * 2.5) #RRR set to 2.5:1
                        ...
                        for potencial_void in range(5):
                            if df_hotovy.candle_size_pip[valid_range_index + i + potencial_void] > (
                                    range_size * void_magnitude):
                                signal = "SELL {} SL {} TP {}".format(df_hotovy.floor_price[valid_range_index], df_hotovy.roof_price[valid_range_index] + 0.00020,
                                                                      df_hotovy.floor_price[valid_range_index] - (abs(df_hotovy.floor_price[valid_range_index] - (df_hotovy.roof_price[valid_range_index] + 0.00020)))*2.5) #RRR set to 2.5:1
                                return [valid_range_index, signal]
                            break
